...
begin
if ((rol.Text = 'Любая') and (otvetstv.text = '') and (postanov.text = '') and (nazvanie.text = '' ) and (status.text = 'Любой')and (CBDate.Text = 'Любая дата')) then
begin
...
if postanov.text <> '' then
  begin
  if (rol.Text <> 'Любая')or(otvetstv.text <> '') then
    begin
    spost:=' AND ';
    end;
  s1:=TStringList.Create;
...
if nazvanie.text <> '' then
  begin
  if (rol.Text <> 'Любая')or(otvetstv.text <> '') or (postanov.text <> '') then
    begin
    snazv:=' AND ';
    end;
  snazv:=snazv+' nazvanie like '+chr(39)+'%'+nazvanie.text+'%'+chr(39);
  end;
if status.text <> 'Любой' then
  begin
  if (rol.Text <> 'Любая')or(otvetstv.text <> '') or (postanov.text <> '') or (nazvanie.text <> '' ) then
    begin
    sstat:=' AND ';
    end;
  sstat:=sstat+' statuz = '+chr(39)+status.text+chr(39);
  end;
if CBDate.Text = 'Выбрать' then
  begin
  if (rol.Text <> 'Любая')or(otvetstv.text <> '') or (postanov.text <> '') or (nazvanie.text <> '' ) or (status.text <> 'Любой') then
    begin
    sdate1:=' AND ';
    end;
...
S:='SELECT id as ''№ п/п'', rol as ''Роль'', otvetstv as ''Ответственный'', postanov as ''Постановщик'', nazvanie as ''Название'', statuz as ''Статус'', date_okon as ''Дата окончания'' from tasks WHERE'+srol+sotv+spost+snazv+sstat+sdate1;
//Edit1.Text:=S;
FMain.ADOQuery1.SQL.Clear;
